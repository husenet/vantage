...
| `--methods` | Per-method probe; `--active` adds POST/PUT/DELETE/PATCH. Reports the status only, since a label like "blocked" just restates the code. Redirects are not followed, so the status is the one returned at the URL you asked for, annotated with its target (`307 GET -> /auth/login`). A write method whose body matches GET is annotated `same body as GET`, since a 200 there actioned nothing. |
